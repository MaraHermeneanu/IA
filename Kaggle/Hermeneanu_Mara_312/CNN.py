...
class CNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels = 1, out_channels = 60, kernel_size = 3) #kernel = filter size = 3x3 stride=skipping  
        ...
        self.conv3 = torch.nn.Conv2d(in_channels = 120, out_channels = 240, kernel_size = 3) #padding ca imaginea sa nu devina prea mica si sa pastrez info din margini 
...
with torch.no_grad(): #dezactivez calculul gradientilor
    for batch in test_loader: #images
        ...
        predicted_test_labels.extend(torch.max(preds.data, 1)[1].numpy()) # .numpy() tensor->numpy
...
file_test_preds = open("submission-x","w")
