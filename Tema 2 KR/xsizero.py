...
class Joc:
    ...
    def mutari(self, jucator_opus):
        ...
        for i in range(len(self.matr)):
            ...
            ci = i%10 #coloana i            
            for j in range(len(self.matr)):
                if self.matr[i]==self.__class__.GOL and self.matr[j]==self.__class__.GOL: #doi indici liberi
...
#primeste matricea, indicii simbolurilor din matricea desfasurata
def mutare_valida(matrice,np1,np2):

    #construiesc un vector cu indicii
    indici = [np1,np2]
...
def main():
    ...
    while True :
        ...
        if stare_curenta.j_curent==Joc.JMIN:
            if optjoc =="PC vs PC":
                #Mutare calculator cu a doua estimare
